...
impl TitleManager {
    ...
    pub fn update(&mut self, state: &TitleState<'_>) -> Option<String> {
        ...
        if !has_parts {
            self.composed.clear();
            self.composed.push_str(PRODUCT_CLI);
        }
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape(PRODUCT_CLI);
        self.last_title.clear();
        self.last_title.push_str(PRODUCT_CLI);
...
mod tests {
    ...
    fn grok_only_produces_just_grok() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, PRODUCT_CLI);
    }
    ...
    fn session_name_and_grok_joined_with_separator() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, format!("my project - {PRODUCT_CLI}"));
    }
    ...
    fn spinner_only_shown_when_active() {
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(&format!(" - {PRODUCT_CLI}")));
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, format!("Waiting - {PRODUCT_CLI}"));
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, format!("Thinking - {PRODUCT_CLI}"));
    }
    ...
    fn model_item_shown_when_present() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, format!("grok-3 - {PRODUCT_CLI}"));
    }
    ...
    fn turn_timer_shown_when_above_one_second() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, format!("42s - {PRODUCT_CLI}"));
    }
    ...
    fn reset_clears_state_and_emits_grok() {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, PRODUCT_CLI);
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(
                t.contains(PRODUCT_CLI),
                "title missing '{PRODUCT_CLI}': {t}"
            );
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            format!("Thinking - proj - grok-3 - workspace - {PRODUCT_CLI}")
        );
    }
