//! `groky mcp`: manage MCP server configurations from the command line.
...
Examples:
  ...
  groky mcp add xcode -- xcrun mcpbridge

  # Add a stdio server with environment variables
  groky mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

  # Add a remote HTTP server
  groky mcp add --transport http sentry https://mcp.sentry.dev/mcp

  # Add a remote server with an authentication header
  groky mcp add --transport http api https://mcp.example.com/mcp --header \"Authorization: Bearer YOUR_TOKEN\"

  # Add to the project config (./.grok/config.toml) instead of ~/.grok/config.toml
  groky mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: groky mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: groky mcp add --transport {label} <name> <url>"
                );
            };
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    if !mcp_server_is_known(name, &cwd) {
        ...
        if !available.is_empty() {
            ...
            eprintln!("No MCP servers configured. Run `groky mcp add --help` to get started.");
        }
        std::process::exit(1);
    }
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!("Specify which one to remove, e.g.: groky mcp remove {name} --scope project");
            std::process::exit(1);
        }
    };
