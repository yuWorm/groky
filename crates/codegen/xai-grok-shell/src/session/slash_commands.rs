...
pub(super) const BUILTIN_COMMANDS: &[BuiltinCommand] = &[
    ...
    BuiltinCommand {
        name: "window",
        description: "Set this session's context-window gear (256k, 512k, max)",
        argument_hint: Some("256k|512k|1024k|max"),
        aliases: &[],
        model_authored_eligibility: ModelAuthoredEligibility::Denied,
        gate: BuiltinGate::AlwaysOn,
        workflow_projection: WorkflowProjection::None,
        resolve: |args| {
            let spec = args.trim();
            BuiltinAction::SetContextWindow {
                spec: (!spec.is_empty()).then(|| spec.to_string()),
            }
        },
    },
    BuiltinCommand {
...
impl BuiltinAction {
    pub(crate) fn command_name(&self) -> &'static str {
        match self {
            ...
            BuiltinAction::SetContextWindow { .. } => "window",
    ...
    pub(crate) fn args_provided(&self) -> bool {
        match self {
            ...
            BuiltinAction::SetContextWindow { spec } => spec.is_some(),
